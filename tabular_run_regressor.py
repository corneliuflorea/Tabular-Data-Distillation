"""
Dataset experiments runner for Linux/Windows
...
BASE_FOLDER = os.path.join(".", "datasets/Regression")
RANDOM_STATE = 42
log_file  = "basic_regressor_distil.log"
...
def  get_metrics(y_test, y_pred,  params) : 
...
def run_one_param(idx, params, X_train, y_train, X_test, y_test,  num_labels, RANDOM_STATE):
	...
	mse, pear = get_metrics(y_test, y_pred,  params)
	return idx, mse, pear


def run_svm_grid(X_train, y_train, X_test, y_test):
    """
    ...
    results = {	"mse": results_mse, "pearson": results_pear	}
    best = { "mse": np.min(results_mse), "pearson": np.max(results_pear)	}

    return {"all": results, "best": best}



def run_one_rf_param(idx, frac, mf, X_train, y_train, X_test, y_test, num_labels, RANDOM_STATE):
...
def run_rf_grid(X_train, y_train, X_test, y_test):
...
def run_xgb_grid(X_train, y_train, X_test, y_test):
    """
    XGBoost grid:
...
def process_one_dataset(dataset_path: str, key_string: str, results_string: str):
    ...
    # decide task type
    print(f"  - Detected task type: Regression")
    with open(log_file, "a") as f:
        print(f"  - Detected task type: Regression", file=f)
    ...
    results = np.append(results, {results_string: {"meta": meta, "regressor": {}}} )
    ...
    # ~ svm_res = run_svm_grid(X_tr, y_tr, X_te, y_te)
    # ~ results[len(results)-1][results_string]["regressor"]["SVM"] = svm_res

    # ~ # Random Forest
    # ~ print("  - Running Random Forest grid ...")
    # ~ with open(log_file, "a") as f:
        # ~ print("  - Running Random Forest grid ...", file=f)
    # ~ rf_res = run_rf_grid(X_tr, y_tr, X_te, y_te)
    # ~ results[len(results)-1][results_string]["regressor"]["RF"] = rf_res
    ...
    xgb_res = run_xgb_grid(X_tr, y_tr, X_te, y_te)
    results[len(results)-1][results_string]["regressor"]["XGB"] = xgb_res
    # ~ pdb.set_trace()
